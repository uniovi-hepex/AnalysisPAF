...
ClassImp(ttDM);
ttDM::ttDM() : PAFChainItemSelector() {
  fTree = 0;
  ...
  TMT2 = 0; TMll = 0;  TMET = 0; TMET_Phi = 0; TgenMET = 0; TNJets = 0; TNBtags = 0; THT = 0; TDeltaPhi = 0;
   ...
   // if(1){  // Veto to 3rd lepton
    if(TNSelLeps == 2){  
      ...
      //if((selLeptons.at(0).p + selLeptons.at(1).p).M() > 20){ // mll > 20 GeV
        if(gChannel == 1 || (TMath::Abs((selLeptons.at(0).p + selLeptons.at(1).p).M() - 91) > 20)  ){ //  Z Veto in ee, µµ
...
//  if(sampleName == "TTbar_PowhegLHE") fTree->Branch("TLHEWeight",      TLHEWeigjt,      "TLHEWeight[TNLHEWeight]/F");
}

void ttDM::GetLeptonVariables(std::vector<Lepton> selLeptons, std::vector<Lepton> VetoLeptons){
...
void ttDM::GetJetVariables(std::vector<Jet> selJets, std::vector<Jet> cleanedJets15, Float_t ptCut){
...
void ttDM::GetDeltaPhi(){
    if(TNSelLeps > 1){
      vmet.SetPtEtaPhiM(TMET, 0, TMET_Phi, 0);
      TDeltaPhi= vmet.DeltaPhi(selLeptons.at(0).p+selLeptons.at(1).p+vmet);
    }
    //for(Int_t i = 0; i < TNLHEWeight; i++) TLHEWeight[i] = 0;
}
