...
float leptonScaleFactor(int pdgId, float pt, float eta, float ht) {
  if (TMath::Abs(pdgId)==13) return muonScaleFactor(pt, eta)*trackingSF(eta);
...
float leptonScaleFactor_err(int pdgId, float pt, float eta, float ht) {
  if (TMath::Abs(pdgId)==13) return muonScaleFactor_error(pt, eta);
  else if (TMath::Abs(pdgId)==11){
...
double Ele23_Ele12_DZLeg(double eta1, double eta2){
  ...
  if(fabs(eta1)>2.5) return 0.;
  if(fabs(eta2)>2.5) return 0.;
...
double MuX_EleY_DZLeg(double eta_e, double eta_mu){
  if(fabs(eta_e)>2.5) return 0.;
  if(fabs(eta_mu)>2.5) return 0.;
...
double HTLeg_SF(int lep1_pdgId, double lep1_pt, int lep2_pdgId, double lep2_pt,  double HT){
  if(TMath::Abs(lep1_pdgId)!=13 || TMath::Abs(lep2_pdgId) !=13) return 1 ;// HT ineffcy  only measured for dimuon+HT. 
  if(HT<300) return 1.;
  if(HT>1000) return 1.;
...
double TotalTriggerSF(int lep1_pdgId, double lep1_pt, double lep1_eta, int lep2_pdgId, double lep2_pt, double lep2_eta, double HT){
  ...
  if(TMath::Abs(lep1_pdgId)+ TMath::Abs(lep2_pdgId) ==22) {//ele-Ele
  ...
  else if(TMath::Abs(lep1_pdgId)+ TMath::Abs(lep2_pdgId) ==26) {//mu-mu
  ...
  else if(TMath::Abs(lep1_pdgId)+ TMath::Abs(lep2_pdgId) ==24) {//ele-mu
    if(TMath::Abs(lep1_pdgId)==11){//leading ele
