...
void DrawPlot(TString var, TString cut, TString chan, Int_t nbins, Float_t bin0, Float_t binN, Float_t* bins, TString Xtitle, TString name){
  ...
  p->SetOutputName("");
  p->SetPlotFolder("output/Systematics/ISR/");
  p->verbose = false;  p->doYieldsInLeg = false;
  p->SetLegendTextSize(0.055); p->SetLegendPosition(0.40, 0.70, 0.93, 0.93);
  p->SetVarName(name);
  ...
  Float_t RatioMax = 1.1; Float_t RatioMin = 0.9;
