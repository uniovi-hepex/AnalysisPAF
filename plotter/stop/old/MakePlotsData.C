...
//TString pathToTree = "/nfs/fanae/user/juanr/AnalysisPAF/StopTrees/may13/";
...
void DrawPlots(TString chan = "ElMu"){
  ...
  DrawPlot("TLep_Pt[0]", BaselineCut, chan, 25, 25, 250, "Leading lepton p_T [GeV]", "Lep0Pt_bl");
...
void DrawPlot(TString var, TString cut, TString chan, Int_t nbins, Float_t bin0, Float_t binN, TString Xtitle, TString name){
  ...
  p->AddSample("WZ, WW, ZZ", "VV", itBkg, kYellow-10, 0.50);
  //p->AddSample("WJetsToLNu_aMCatNLO", "WJets", itBkg, kGray, 0.5);
  //p->AddSample("WJetsToLNu_MLM", "WJets", itBkg, kGray, 0.5);
	p->AddSample("TTWToLNu, TTWToQQ, TTZToQQ, TTZToLLNuNu", "ttV", itBkg, kOrange-3, 0.5);
	p->AddSample("DYJetsToLL_M50_aMCatNLO, DYJetsToLL_M10to50_aMCatNLO", "DY", itBkg, kAzure-8, 0.50);
	p->AddSample("TW, TbarW", "tW", itBkg, kMagenta, 0.2);
	p->AddSample("TTbar_Powheg", "ttbar", itBkg, kRed+1, 0.05, "0", "AllInstances");
...
//  p->AddSample("T2tt_250_50_FS_summer", "S_250_50", itSignal, kPink-1,  0.1, "0", "isrweight"); 
//  p->AddSample("T2tt_200_50_FS_summer", "S_200_50", itSignal, kPink-1,  0.1, "0"); 
...
*/
  ...
  p->PrintSystYields("","","","txt,html,tex");
  delete p;
}
