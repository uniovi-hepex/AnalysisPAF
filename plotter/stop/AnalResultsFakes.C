...
//=== Constants...
TString BaselineCut = "TNJets >= 2 && TNBtags >= 1 && TPassTrigger && TPassMETfilters && TNSelLeps == 2 && TLep0_Pt >= 25"; // !TIsSS
...
void AnalResultsFakes(){
  ...
  // Draw MT2 plot, signal plot, yields...
  name = "MT2_21"; mp->SetPlot(name, "M_{T2} (GeV)", "Events"); SetSystematics(); SavePlot();
...
void SetMultiPlot(){
  ...
  mp->AddDistribution("NJets",   "TNJets",   cut, chan,  9, 0, 8);
  ...
  //mp->SetCut(cut + " && TEventTruth == 0 || TEventTruth == 1 || TEventTruth == 3"); // 3 --> flips, 
  weight = "TWeight*(TEventTruth == 0 || TEventTruth == 1 || TEventTruth == 3)*(!TIsSS)"; // 3 --> flips, 
  ...
  //p->SetCut(cut + " && TEventTruth != 0 && TEventTruth != 1 && TEventTruth != 3"); // 3 --> flips, 
  weight = "TWeight*(!TIsSS)*(TEventTruth != 0 && TEventTruth != 1 && TEventTruth != 3)";
	mp->AddSample("DYJetsToLL_M50_aMCatNLO, DYJetsToLL_M10to50_aMCatNLO", "Nonprompt",        itBkg, kGray+1,   systematics, weight);
	mp->AddSample("TW, TbarW",                                            "Nonprompt",        itBkg, kGray+1,   systematics, weight); // TColor::GetColor("#ffcc33")
	mp->AddSample("TTbar_Powheg",                                         "t#bar{t} fakes",     itBkg, kGray+3, systematics, weight);
  weight = "TWeight*!TIsSS";
...
void SavePlot(){
  ...
  if(name.Contains("NBtagsNJets")) mp->SetBinLabels("[0|0], [1|0], [1|1], [2|0], [2|1], [2|2], [3|0], [3|1], [3|2], [3|3], [4|0], [4|1], [4|2], [4|3], [4|4], [>4|]");
    ...
    mp->SetLegendPosition(0.4, 0.65, 0.7, 0.93);
  }
