...
TString systematics = "Trig,JES,Btag,MisTag,ElecEff,MuonEff,PU,MuScale,ElScale,uncl";

void DrawPlotsM(TString chan = "ElMu"){
...
void DrawSignalPlot(TString var, TString cut, TString chan, Int_t nbins, Float_t bin0, Float_t binN, Float_t* bins, TString Xtitle, TString name){
  ...
  //p->AddSample("T2tt_mStop175_mLsp1",   "Stop[175, 1]",    itSignal, kGreen+1);
  p->AddSample("T2tt_225_50_FS_summer", "T2tt[225,50]", itSignal, kCyan); 
  ...
  p->SetSignalProcess("T2tt[225,50]");
  p->SetSignalStyle("SM");
  p->SetRatioMin(0.0);
  p->SetRatioMax(0.2);
