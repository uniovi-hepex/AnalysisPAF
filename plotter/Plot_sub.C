...
Histo* Plot_sub::GetH(TString sample, TString sys, Int_t type){ 
...
void Plot_sub::AddSample(TString p, TString pr, Int_t type, Int_t color, TString sys, TString options){
  //>>> Multiprocess...
  p.ReplaceAll(" ", "");
  if(pr == "") pr = p;
  if(p.Contains(",")){
...
void Plot_sub::GetStack(){ // Sets the histogram hStack
...
Histo* Plot_sub::GetAllBkgClone(TString newname){
  if(!hStack) GetStack();
...
Float_t Plot_sub::GetAllBkg(Int_t ibin){
  if(!hStack) GetStack();
  Float_t val = ibin < 0 ? hAllBkg->GetYield() : hAllBkg->GetBinContent(ibin);
  return val;
}

void Plot_sub::SetData(){  // Returns histogram for Data
  if(!doData){
    GetStack();
...
void Plot_sub::AddToHistos(Histo* p){ // Add the histogram to the right vector
  if     (p->GetType() == itBkg   ) VBkgs.push_back(p);
    ...
    cout << "[Plot_sub::AddToHisto] ERROR: Wrong histo type!!!" << endl;
    return;
  }
  if(verbose) cout << "[Plot_sub::AddToHistos] Added histogram " << p->GetName() << " to " << p->GetTag() << " group (" << p->GetType() << ")" << endl;
}
...
void Plot_sub::AddSystematic(TString var, TString pr){
  if(var == "stat"){
    AddToSystematicLabels("stat");
    AddStatError();
    if(verbose) cout << "[Plot_sub::AddSystematic] Added statistical uncertainties! " << endl;
    return;
  }
  var.ReplaceAll(" ", "");
  if(var.Contains(",")){
  ...
  if(verbose) cout << "[Plot_sub::AddSystematic] Systematic histograms added to the list for variation: " << var << endl;
}


void Plot_sub::AddNormSyst( TString process, TString name, float norm)
{
...
void Plot_sub::AddLumiSyst( float unc )
...
void Plot_sub::AddStatError(TString process){
  TString pr;
  if(process == ""){
...
void Plot_sub::GroupSystematics(){
...
void Plot_sub::Multiloop(TString p, TString pr, Int_t type, Int_t color, TString sys){
  TString pathToMiniTree = path;
  TString sample = p;
  if     (type == itSignal || sample.Contains("T2tt")) pathToMiniTree = pathSignal;
  else if(type == itData  ) pathToMiniTree = pathData;
...
void Plot_sub::PrepareHisto(vector<Histo*> vH, TString sampleName, TString pr, Int_t type, Int_t color, TString sys){
...
void Plot_sub::PrepareHisto(Histo* h, TString sampleName, TString pr, Int_t type, Int_t color, TString sys){
...
void Plot_sub::AddHistoFromFile(TString filename, TString name, int type, int color, TString sys, TString pathInRootfile){
...
void Plot_sub::AddHistoFromFileTGraph(TString filename, TString name, int type, int color, TString sys, TString pathInRootfile){
  if(!filename.EndsWith(".root")) filename += ".root";
  Histo* histo;
  if(x0 == xN) histo = new Histo(TH1F(name, name, nb, vbins));
  else         histo = new Histo(TH1F(name, name, nb, x0, xN));
  ...
  if(type == itBkg){
    ...
    for(i = 0; i < n; i++){
      ...
      if(t == tag){
        ...
        if(verbose) cout << "[Plot_sub::AddToHistos] Added histogram " << h->GetName() << " to " << h->GetTag() << " group (" << type << ")" << endl;
        return;
      }
    }
  }
...
Float_t Plot_sub::GetData(Int_t ibin){
  if(!hData) SetData();
...
Histo* Plot_sub::GetHData(){
  if(!hData) SetData();
  return hData;
}

Histo* Plot_sub::GetHisto(TString pr, TString systag){
  ...
  //cout << "[Plot_sub::GetHisto] WARNING: No systematic " << systag << " for process " << pr << "........... Returning nominal histo..." << endl;
  return GetHisto(pr, "0");
}

Int_t Plot_sub::GetProcessType(TString pr, TString systag){
  ...
  if(verbose) cout << "[Plot_sub::GetProcessType]: Not found process " << pr << endl;
  return -1;
}

Int_t Plot_sub::GetProcessIndex(TString pr, TString systag){
...
void Plot_sub::RemoveProcess(TString pr, TString systag){
  Int_t type = GetProcessType(pr, systag);
  ...
  if     (type == itBkg)    VBkgs.erase(   VBkgs.begin() + pos); 
    ...
    if(verbose) cout << "[Plot_sub::RemoveProcess]: Not found process " << pr << ", with syst " << systag << endl;
    return;
  }
  if(systag == "" || systag == "0"){
...
Bool_t Plot_sub::Exists(TString pr, TString systag){
  if(GetProcessIndex(pr, systag) == -1) return false;
  else return true;
}

Bool_t Plot_sub::RenameProcess(TString pr, TString newname, TString systag){
...
void Plot_sub::GroupProcesses(TString pr, TString newProcess){
  if(pr.Contains(",")){
  ...
  if(!Exists(pr)){
    cout << "[Plot_sub::GroupProcesses] ERROR: process \"" << pr << "\" does not exist!" << endl;
    return;
  }
...
Histo* Plot_sub::GetHistoError(TString pr, TString syst){
...
void Plot_sub::AddUncToHisto(TString pr, TString sys, TString pr_new, TString sys_new){
...
Histo* Plot_sub::GetSymmetricHisto(TString pr, TString systag){
...
void Plot_sub::AddSymmetricHisto(TString pr, TString systag){
  Histo* h = GetSymmetricHisto(pr, systag);
  AddToHistos(h);
}

Float_t Plot_sub::GetYield(TString pr, TString systag, Int_t ibin){
  ...
  //cout << "[Plot_sub::GetYield] WARNING: No systematic " << systag << " for process " << pr << "!!! ...... Returning nominal value... " << endl;
  return GetYield(pr, "", ibin);
}

TLegend* Plot_sub::SetLegend(){ // To be executed before using the legend
...
void Plot_sub::SetLegendPosition(TString pos)
{
  if (pos=="UR")
    ...
    cout << "[Plot_sub::SetLegendPosition] Sorry, position " << pos 
...
TCanvas* Plot_sub::SetCanvas(){ // Returns the canvas
  ...
  texcms->SetTextSizePixels(texCMSsize); // 22
  ...
  texPrelim->SetTextSizePixels(texPrelimSize); // 22

  return c;
}

void Plot_sub::DrawComp(TString tag, bool doNorm, TString options){
  doUncInLegend = false;
...
void Plot_sub::DrawStack(TString tag){
  std::vector<Histo*> VStackedSignals;
  if(verbose) cout << "[Plot_sub::DrawStack] Setting Canvas..." << endl;
  TCanvas* c = SetCanvas(); plot->cd(); 
  GetStack();
  SetData();
...
/*  if((SignalStyle == "SM" || SignalStyle == "H")){ 
  ...
  if(RatioYtitle == "S/B"){
    cout << "Ratio: S/B!\n";
    if(!doSignal) cout << "[Plot_sub::DrawStack] WARNING: cannot print ratio Signal/Background without signal!!" << endl;
    else{
  ...
  /*    for(int bin = 1; bin <= nbins; bin++){  // Set bin error
    ...
    //if(!doData) cout << "[Plot_sub::DrawStack] WARNING: cannot print ratio Data/MC without data!!" << endl;
    //else{
...
void Plot_sub::ScaleProcess(TString pr, Float_t SF){
...
void Plot_sub::ScaleProcessBin(TString pr, Float_t SF, Int_t ibin){
  float binContent; 
  for(Int_t i = 0; i < (Int_t) VBkgs.size(); i++) if(VBkgs.at(i)->GetProcess() == (pr)){
...
void Plot_sub::ScaleSignal(Float_t SF){
  ScaleProcess(SignalProcess, SF);
}

void Plot_sub::ScaleSys(TString pr, TString sys, Float_t SF){
...
void Plot_sub::SetGoF(TString thegof)
{
...
void Plot_sub::SetPad(TPad* pad, TString limits, TString margins, bool doGrid){
...
void Plot_sub::SetHRatio(TH1F* h){
...
void Plot_sub::SetAxis(TAxis *a, TString tit, Float_t titSize, Float_t titOffset, Int_t nDiv, Float_t labelSize){
  a->SetTitle(tit);
...
void Plot_sub::SetYaxis(TAxis *a){
...
Int_t Plot_sub::GetColorOfProcess(TString pr){
...
void Plot_sub::RemoveSystematic(TString sys){
  if(sys.Contains(",")){
...
void Plot_sub::UseEnvelope(TString pr, TString tags, TString newname){
...
Plot_sub* Plot_sub::NewPlot(TString newVar, TString newCut, TString newChan, Int_t newnbins, Float_t newbin0, Float_t newbinN, TString newtitle, TString newXtitle){
  ...
  Plot_sub* p = new Plot_sub(newVar, newCut, newChan, newnbins, newbin0, newbinN, newtitle, newXtitle);
...
void Plot_sub::PrintSystematics(){
...
Float_t Plot_sub::GetTotalSystematic(TString pr){
  Float_t sys2 = 0;
  Int_t nSys  = VSystLabel.size();
...
void Plot_sub::PrintYields(TString cuts, TString labels, TString channels, TString options){
  if(cuts == "") cuts = cut;
  //cuts.ReplaceAll(" ", ""); 
...
void Plot_sub::PrintBinsYields(TString options){
  Int_t nBkgs = VBkgs.size();
  Int_t nSignals = VSignals.size();
  Int_t nrows = nBkgs + nSignals; 
  if(nBkgs > 0) nrows++;
...
void Plot_sub::AddNormUnc(TString pr, Float_t systUp, Float_t systDown){
...
Float_t Plot_sub::GetBinYield(TString pr, Int_t bin, TString systag){
...
void Plot_sub::AddPlotFromHyperlooper(Hyperlooper *HyperLoop, TString plotname){
...
void MultiPlot_sub::AddHyperlooper(TString sample, TString process, Int_t type, Int_t color, TString syst, TString weight){
  nDist = Dname.size(); 
...
void MultiPlot_sub::AddDistribution(TString name, TString var, TString cut, TString chan, Int_t nbins, Float_t bin0, Float_t binN, Float_t *bins){
...
void MultiPlot_sub::AddSample(TString p, TString pr, Int_t type, Int_t color, TString syst, TString weight){
  p.ReplaceAll(" ", "");
  if(pr == "") pr = p;
  if(p.Contains(",")){
    TString First_p = p(0, p.First(','));
...
void MultiPlot_sub::SetPlot(TString name, TString xtitle, TString ytitle, TString seltext, Float_t ratioMax, Float_t ratioMin){
  Reset();
  Loop();
  nSamples = VTagSamples.size();
  for(Int_t i = 0; i < nSamples; i++) 
