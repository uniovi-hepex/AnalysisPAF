...
Float_t t4fake(float pt, float eta, int id){
  if(id < 0) id = -id;
  if(id == 11) return electronFakeRate(pt, eta);
  else         return muonFakeRate(    pt, eta);
  return 0.;
}

Float_t t4fake_err(float pt, float eta, int id){
  if(id < 0) id = -id;
  if(id == 11) return electronFakeRateError(pt, eta);
  else         return muonFakeRateError(    pt, eta);
  return 0.;
}
