...
//  p->doData = false;
  ...
  if (NameOfTree == "top"){//Load MC FAKES
    p->SetCut(cut + " && TConvNumber > 0 && TFakeNumber == 0"); //Allow Conversions
    p->AddSample("TGJets, TTGJets, WGToLNuG, ZGTo2LG, WZG_amcatnlo, WWG_amcatnlo",                "X+#gamma", itBkg, kViolet+2);  // X+gamma 
  
    p->SetCut(cut + " && TConvNumber == 0 && TFakeNumber == 0"); //Veto Conversions
    ...
    p->SetCut(cut + " && TConvNumber == 0"); //Add the fakes
  ...
  if (NameOfTree != "top"){//Load DATA FAKES, APPLY FAKE RATE FILES!!!
    ...
    p->SetWeight("GetFRweightlepMVAM3lep(GetFRweightMVAM(TLep_PtZ1, TLep_EtaZ1, TLep_IsTightZ1, TLep_pdgIdZ1), GetFRweightMVAM(TLep_PtZ2, TLep_EtaZ2, TLep_IsTightZ2, TLep_pdgIdZ2), GetFRweightMVAM(TLep_PtW, TLep_EtaW, TLep_IsTightW, TLep_pdgIdW), 1)"); //Fake rates for the fakerate god  
    //DATA FAKES
